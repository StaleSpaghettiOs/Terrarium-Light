...
void setup() {
  ...
  configTime(3600 * 12, 0, "time.nist.gov", "www.pool.ntp.org/zone/nz"); // GMT +12 Hrs
}
...
void loop() {
  ...
  if (timeSet){
    currentTime -= 60; // Workaround to minus 7 hours from timezone
    adjustLEDs();
  }
}
...
void fadeDownToBrightnessIn(uint8_t _brightness, uint16_t _ms) {
  while (FastLED.getBrightness() > _brightness){
    FastLED.setBrightness(FastLED.getBrightness() - 1);
    FastLED.show();
    yield();                // ** Workaround **
...
void adjustLEDs() {
  ...
  if (prevMinute != minute) {
    ...
    if (hourOfDay < 5) {
      ...
      fadeUpToBrightnessIn(63 + (minute / 59.0) * 64, 10000);
    }
    else if (hourOfDay < 8) {
      fadeUpToBrightnessIn(127 + (minute / 59.0) * 64, 10000);
    }
    else if (hourOfDay < 9) {
      fadeUpToBrightnessIn(191 + (minute / 59.0) * 64, 10000); // Full bright by 9 AM
    }
    else if (hourOfDay < 17) {
      fadeUpToBrightnessIn(255, 1000);
    } // Nothing to do till 4 PM
    else if (hourOfDay < 18) {
      fadeDownToBrightnessIn(191 - (minute / 59.0) * 64, 10000);
    }
    else if (hourOfDay < 19) {
      fadeDownToBrightnessIn(127 - (minute / 59.0) * 64, 10000);
    }
    else if (hourOfDay < 20) {
      fadeDownToBrightnessIn(63 - (minute / 59.0) * 64, 10000);
    }
    else {
      fadeToBlackIn(10000);
    }
